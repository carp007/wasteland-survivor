// -------------------------------------------------------------------------------------------------
// GamePawnKit
// File: PawnKit/Pawns/HumanoidPawn.cs
// Purpose: Reusable on-foot humanoid pawn (CharacterBody3D) with optional avatar/animation support.
// -------------------------------------------------------------------------------------------------
using System;
using Godot;
...
/// <summary>
/// Minimal on-foot pawn suitable for top-down / 2.5D games.
///
/// - Top-down movement on the XZ plane.
/// - Uses CharacterBody3D for simple collision against world geometry.
/// - Input is typically provided by a controller (player or AI) via <see cref="MoveInput"/> and <see cref="Sprint"/>.
/// </summary>
public partial class HumanoidPawn : CharacterBody3D, IHumanoidPawn
{
	[Export] public float MoveSpeed { get; set; } = 7.0f;
	[Export] public float SprintMultiplier { get; set; } = 1.6f;

	// Optional avatar/animation configuration.
	public HumanoidAvatarConfig AvatarConfig { get; set; } = HumanoidAvatarConfig.Default();
	...
	public virtual void TriggerDeath()
	{
		...
		GD.Print("[HumanoidPawn] TriggerDeath: no death animation found; using fallback corpse pose.");
	}
	...
	private void TryLoadAvatarFromConfig()
	{
		...
		var cfg = AvatarConfig ?? HumanoidAvatarConfig.Default();
		...
		try
		{
			...
			{
				GD.PushWarning($"[HumanoidPawn] Avatar scene not found at {cfg.ModelScenePath}. Using capsule placeholder.");
				return;
			}
			...
			if (ps == null)
			{
				GD.PrintErr($"[HumanoidPawn] Failed to load avatar scene: {cfg.ModelScenePath}");
				return;
			}
			...
			{
				GD.PrintErr($"[HumanoidPawn] Avatar scene is not a Node3D: {cfg.ModelScenePath}");
				return;
			}
			...
			_avatarRoot.Position = cfg.ModelLocalOffset;
			...
			if (_avatarAnimPlayer == null)
			{
				GD.Print($"[HumanoidPawn] Avatar loaded but no AnimationPlayer found under {cfg.ModelScenePath}. Movement will be unanimated.");
			}
			else
			{
		...
		{
			GD.PrintErr($"[HumanoidPawn] Failed to load avatar: {ex.Message}");
		}
	}

	private void ResolveAnimationNames(HumanoidAvatarConfig cfg)
	{
	...
	private void LogAvatarAnimationNamesOnce()
	{
		...
		try
		{
			...
			GD.Print($"[HumanoidPawn] Avatar animations: {sb}");
			GD.Print($"[HumanoidPawn] Resolved idle={_resolvedIdle ?? "<none>"}, walk={_resolvedWalk ?? "<none>"}, run={_resolvedRun ?? "<none>"}, death={_resolvedDeath ?? "<none>"}");
		}
		catch
		{
			// ignore
	...
	private string? ResolveAnimName(string desired)
	{
		...
		{
			var name = sn.ToString();
			if (name.IndexOf(desired, StringComparison.OrdinalIgnoreCase) >= 0)
				return name;
		}
		return null;
	}
