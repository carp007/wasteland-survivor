- Build 2026-07-22-188 (8-hour loop #6, iteration 11 — closing-regrade response; visual moved B- → B, menu-family and arena-combat both B+):
  ...
  - **On-foot/duelist pawns finally visible** (judge P1): identity rings grew (0.50→0.64 outer) and brightened (~2x emission), plus a beacon tick above head height — the "near-invisible black speck" read is gone for both the player's gold and hostiles' red.
  - **Highway reads OUTDOORS** (judge P1): open-road fights get a burnt-dusk horizon, warm ambient (0.52), a low raking dusk sun with real shadows, and NO broadcast light towers — the "warehouse with a road in it" lighting is gone.
  - **t5 washout trimmed** (judge P1): stacked tier-scaled light energies at SpectacleT=1 ran the pit-05 frame to white/amber — corner pools 6.6→5.2, field wash factor 2.2→1.5.

- Build 2026-07-22-187 (8-hour loop #6, iterations 9-10 — wasteland outfitting, duel mercy, travel texture):
  - **WASTELAND OUTFITTING** (play-test: cars "a bit cartoony"; judge: "clean-plastic-simple at combat zoom" — via implementation agent): every live combat vehicle now carries procedural per-class greebles — tube bullbars (sedans/SUVs/compacts/vans) or rust-ribbed ram plates (trucks/industrial), roof racks with strapped crates/duffels or exhaust stacks, whip antennas, rocker skirts + mud flaps, tow hooks, side-offset spare tires on compact/SUV. Deterministic per (def + model), matte hull-detailer palette, primitives only; trailers and derelict dressing untouched; identity-VFX/HUD anchors byte-identical (outfitting excluded from AABB measurements). Verified A/B at arena zoom, garage portrait, and store thumbnails — all six chassis thumbnails improved.
  - **Duelist mercy rule**: a bailed-out driver shot below ~25% HP surrenders (hands up, fight over, hull forfeit) — the player isn't forced to execute a beaten opponent; kill or surrender both leave the hull salvage-whole.
  - **Journey FUEL STOP beat**: legs with a planned mid-route top-up now pause at the pumps (~40% distance, road freezes, cyan "FUEL STOP — TOP-UP -$" banner) — the travel Stage 3 agency direction, presentation-first.
  - **Shared ammo-pool carry cap**: pool caps are now the largest capacity among OWNED weapons feeding that ammo id (your gear defines your webbing) — stable across equip-switches; store rows and buy buttons use it (the old equipped-weapon cap let a pool "exceed" its displayed cap after switching SMG→pistol).
  - **Parts-store backdrop presence** (play-test note 5): backdrop dim lifted 0.30→0.52 so the city art actually reads behind the drawer.
